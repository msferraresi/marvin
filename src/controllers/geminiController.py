...
class GeminiController:
    ...
    def ask_gemini_for_intent(cls, text, opciones=""):
        ...
        for attempt in range(cls.MAX_RETRIES):
            try:
                logger.info(
                    f"Intento {attempt + 1} de {cls.MAX_RETRIES} para obtener intent."
                )
                ...
                if attempt < cls.MAX_RETRIES - 1:
                    logger.debug(f"Reintentando en {cls.RETRY_SLEEP} segundos...")
    ...
    def ask_gemini_for_response(cls, text, opciones="."):
        ...
        for attempt in range(cls.MAX_RETRIES):
            try:
                logger.info(
                    f"Intento {attempt + 1} de {cls.MAX_RETRIES} para obtener respuesta."
                )
                model = gemini.GenerativeModel("gemini-1.5-flash")
                response = model.generate_content(prompt)
